...
指导 Agent 设计和补写测试用例；放通用测试思维，不放具体框架 API
...
- 用用户可见行为命名，不用实现细节命名
- 一个用例验证一个主要行为；共享 setup 可抽，不为抽象而抽象
- 优先 Arrange / Act / Assert 或 Given / When / Then 结构
- 断言用户可观察结果：文本、状态、URL、请求结果、可访问性状态
- Mock 只 mock 边界外依赖；不 mock 被测逻辑本身
...
- 用 E2E 测大量纯逻辑分支
- 一个用例断言多个不相关行为
- 为让测试通过改业务行为但不回 `frontend-implement`
- 用固定等待替代可观察条件等待
...
1. 先读 `docs/technical-architecture.md` → **测试**，确认框架、命令、目录和项目约定
2. 参考项目已有测试示例，优先保持一致
3. 框架 API、mock、异步等待、E2E locator 等细节 → 查当前官方文档
4. 不凭过期记忆硬写框架语法
