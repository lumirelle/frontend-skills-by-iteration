...
2. 读取对应 summarized 与 design，确认状态均为 `ACTIVE`，再确认需求、验收标准、推荐方案、涉及文件。
...
8. **状态门禁**：不得消费 `DRAFT`、`STALE`、`BLOCKED` summarized 或 design；有阻塞 open questions 的 plan 保持 `DRAFT` 或标记 `BLOCKED`。
