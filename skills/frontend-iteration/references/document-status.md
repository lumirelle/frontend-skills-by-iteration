...
`progress.md` 文首 `状态` 恒为 `ACTIVE`，只表示 resume 事实源有效；迭代完成与否看步骤状态表。
...
| `DRAFT` | 停止，等待用户确认或回到产出步骤完善 | 唯一例外见 [orchestrated-invocation.md](orchestrated-invocation.md) → DRAFT 消费例外；简述：仅 `frontend-iteration fast` 步骤 1→2→3 且 `progress.md` → **草稿批次** 为 `open` 时可消费，进入步骤 4 前必须批量确认转 `ACTIVE` |
...
| 字段 | 体系 | 规则 |
|------|------|------|
| 文首 `状态` | 固定 `ACTIVE` | 仅表示 tracker 有效；迭代是否结束看步骤 7 是否为 `passed` |
...
1. 读取输入文档时先检查 `Status`。
2. 遇到 `DRAFT`、`STALE`、`BLOCKED` 时按 Agent 决策表处理，并向用户说明需要回到哪个上游步骤。
3. 更新文档时同步更新 `更新于` 日期。
4. 标记 `STALE` 时写清 `失效原因`，例如 `origin PRD updated` 或 `design decision changed`。
