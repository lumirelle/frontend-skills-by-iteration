...
**恢复执行时**：先读步骤状态、计划任务状态、草稿批次，再读阻塞项；缺行按上表补全后再继续。存在 `open` 草稿批次 → 只能继续 fast 1–3 或停在步骤 3 末批量确认；不得进步骤 4。输入产物中存在 `DRAFT` 文档但无 `open` 批次 → 遗留 DRAFT：展示请用户确认转 `ACTIVE` 后继续
...
4. fast 1–3 生成或消费 `DRAFT` 产物时须维护草稿批次；批量确认/拒绝/废弃时标 `confirmed` 或 `abandoned`
