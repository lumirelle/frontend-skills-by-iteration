...
| 调用方式 | 规则 |
|----------|------|
| **由 `frontend-iteration` 调用**（orchestrated） | 遵循编排器的 fast / strict 模式、`progress.md` 更新与确认规则；确认时机与 `DRAFT` 消费范围由编排器决定，**不在 sub-skill 内单步等待**（见下「Workflow 变体」） |
...
> **只读单个 sub-skill 时注意**：sub-skill Workflow 写的「等待确认 / 仅消费 ACTIVE」是 **standalone 默认**。由编排器调用时以本表为准——fast 步骤 1–3 **不要因为上游是编排草稿 `DRAFT`、或本步未单步确认而停止**。
